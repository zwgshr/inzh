...
var utils = {
     ...
     */
    e2ten: function (num) {
        var result = this.REG_E.exec(num.toString());
     ...
     * @returns object
     */
    getNumbResult: function (num) {
        var result = this.REG_NUMBER.exec(num.toString());
        if (!result && this.REG_E.test(num.toString())) {
            result = this.REG_NUMBER.exec(this.e2ten(num.toString()))
        }
        if (result) {
            return {
                int: result[2],
     ...
     * @returns array
     */
    centerArray: function centerArray(baseArray, array1 /*[, array2[, ...[, arrayN]]]*/) {
...
var Nzh = {
    ...
    //数组头部插0
    unshift0: function (arr, n, number) {
        if (n == null) n = 1;
        for (; n--;) arr.unshift(number || 0);
    },

    zero_comm: function (str, char_0, type) {
        if (str == null) return "";
    ...
    CL: function (num, options) {
        ...
        var encodeInt = function (_int, _m, _dg) {
            ...
            int = self.zero_comm(int, n0); //修整零
            return int;
        };

        //转换小数部分
        if (_decimal) {
            _decimal = this.zero_comm(_decimal, "0", "$"); //去除尾部0
 ...
 *                         }
    ...
    toMoney: function (num, options) {
        ...
        if (options.complete) {
            ...
            zs_str = this.CL.call(this, _num, options) + this.m_u.charAt(0);
        } else {
            _decimal = this.zero_comm(_decimal, "0", "$");//去除尾部的0
            if (_decimal) {
                var mark_0;
                ...
                //if(_num == "0"){xs_str = this.zero_comm(xs_str,ch_0,"^")}
            }
            if (_num != "0" || zs_str || !xs_str) {
                zs_str = this.CL.call(this, _num, options) + this.m_u.charAt(0) + zs_str;
            }
        }
